...
class Trainer_3DGS(object):
    def __init__(self,
        ...
        if(self.opt.object_ply_path is not None):
            if self.opt.transform_ckpt is not None:
                def load_transform_checkpoint(filename, model, optimizer=None):
                    ...
                    return epoch, loss
                # from personalization.box_learn.clip_box_refine import load_checkpoint
                ...
                self.model.load_ply_from_gs_editor(self.opt.object_ply_path)
                input_data = self.model._xyz
                scale, rotmat, tvec = net(input_data)
                apply_gaussian_transform(self.model, scale, rotmat, tvec - torch.tensor([0.0,0.0,0.0], device=device))
            else:
                self.model.add_from_ply(self.opt.object_ply_path)
            checkpoint_dict = torch.load(self.opt.init_ckpt, map_location=device)
            if isinstance(checkpoint_dict, tuple):
                checkpoint_dict = {'model': checkpoint_dict[0]}
            ...
            if self.workspace is not None:
                if self.use_checkpoint == "scratch":
                    ...
                    if(self.opt.init_ckpt):
                        self.load_checkpoint(self.opt.init_ckpt)
                        self.model.save_ply(os.path.join(self.workspace, 'ply', 'init.ply'))
                    if(self.opt.init_ply):
                        # 如果 model 已经通过外部流程加载了点云（例如 train_3DGS.py 先调用 create_from_pcd），
                        # 则跳过 Trainer 再次调用 load_ply_from_gs_editor，避免重复拼接/掩码长度不匹配。
                        try:
                            has_pts = hasattr(self.model, 'get_xyz') and self.model.get_xyz.numel() > 0
                        except Exception:
                            has_pts = False
                        if has_pts:
                            self.log('[INFO] model already contains points, skipping load_ply_from_gs_editor')
                        else:
                            self.model.load_ply_from_gs_editor(self.opt.init_ply)
                    if(self.opt.ply_to_ckpt):
        ...
        if(self.opt.delete_gs_bbox_path is not None):
            gaussian_box = GSNetwork(self.opt, 'cuda')
            import pymeshlab
            ms = pymeshlab.MeshSet()
            ms.load_new_mesh(self.opt.delete_gs_bbox_path)
            ...
            # apply_gaussian_transform(gaussian_box, scale, rotmat, tvec - torch.tensor([0.0, -0.2,0.0], device=device), float(opt.transform_scale_factor))
            ...
            delete_min_xyz = None
            delete_max_xyz = None   
        if delete_min_xyz is not None:
            self.model.delete_gs_bbox(delete_min_xyz, delete_max_xyz)       
    ...
    def test_step(self, data,  perturb=False, if_gui=False, invert_bg_color=True, p_mask=None):
        ...
        if p_mask is not None:
            outputs = self.model.render(cur_cam,  mask=p_mask, bg_color=bg_color) # invert_bg_color=True,
        else:
        ...
        return all_pred_rgb, all_pred_depth, dir, all_pred_alpha
    ...
    def sample_views_box(self, loader, save_path=None):
        os.makedirs(os.path.join(save_path,'box_rgb'), exist_ok=True)
        pose_dict = {}
        ...
        if self.opt.bbox_path is not None:
            ...
            vertices -= center 
            vertices *= np.array(1.0, dtype=np.float64)
            vertices += center

            mesh.vertex['positions'] = o3d.core.Tensor(vertices, dtype=o3d.core.Dtype.Float32)

            mesh.scale(2.5, center=mesh.get_center())
            ...
            mask = torch.LongTensor(self.inside_check(self.model.get_xyz.data.detach().cpu().numpy(),bounding_box)).to(self.device)
        with torch.no_grad():
            for i, data in tqdm.tqdm(enumerate(loader)):
                ...
                preds, preds_depth, dir, alpha = self.test_step(data,p_mask = mask)  # , invert_bg_color=True, p_mask=trainer.mask
                pred = preds[0].detach().cpu().numpy()
                pred = (pred * 255).astype(np.uint8)
                cv2.imwrite(os.path.join(save_path,'box_rgb', out_name), cv2.cvtColor(pred, cv2.COLOR_RGB2BGR))
    ...
    def sample_views(self, loader, save_path=None):
        ...
        with torch.no_grad():
            for i, data in tqdm.tqdm(enumerate(loader)):
                ...
                radius = data[2]
                out_name = f'{radius}_{theta}_{phi}.png'
                preds, preds_depth, dir, alpha = self.test_step(data)  # , invert_bg_color=True, p_mask=trainer.mask
                pred = preds[0].detach().cpu().numpy()
                pred = (pred * 255).astype(np.uint8)
                ...
                res = cv2.imwrite(os.path.join(save_path, 'depth', out_name), depth)
                # print('res', res)
                pose_dict[out_name] = poses
        np.save(os.path.join(save_path, 'pose_dict.npy'), pose_dict)

    def sample_instance_views(self, loader, save_path=None):
        os.makedirs(os.path.join(save_path,'rgba'), exist_ok=True)
        pose_dict = {}
        with torch.no_grad():
            for i, data in tqdm.tqdm(enumerate(loader)):
                poses = data[-2][0]
                ...
                rgba = cv2.merge([pred_bgr[:, :, 0], pred_bgr[:, :, 1], pred_bgr[:, :, 2], alpha])
                cv2.imwrite(os.path.join(save_path,'rgba', out_name),rgba)
                pose_dict[out_name] = poses
        np.save(os.path.join(save_path, 'pose_dict.npy'), pose_dict)
    ...
    def sample_mask_views(self, loader, save_path=None):
        ...
        with torch.no_grad():
            for i, data in tqdm.tqdm(enumerate(loader)):
                ...
                radius = data[2]
                out_name = f'{radius}_{theta}_{phi}.png'
                preds, preds_depth, dir, alpha = self.test_step(data, if_gui=True)
                pred = preds[0].detach().cpu().numpy()
                pred = (pred * 255).astype(np.uint8)
