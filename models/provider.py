...
from models.colmap_loader import (
    qvec2rotmat,
...
class MiniCam:
    def __init__(self, R, T, width, height, fovy, fovx, pose, znear, zfar):
        ...
        self.world_view_transform = torch.as_tensor(getWorld2View2(R.numpy(), T.numpy(), trans, scale)).transpose(0, 1).cuda()
...
class BaseDataset(Dataset):
    ...
    def _flatten(self, x):
        ...
        if self.batch_type == 'all_images':
            ...
            if self.if_data_cuda:
                x = [torch.as_tensor(y).cuda() for y in x]
            else:
...
class COLMAP(BaseDataset):
    ...
    def _load_renderings(self):
        ...
        for idx, key in enumerate(cam_extrinsics):
            ...
            self.pose_all.append(torch.as_tensor(c2w).float())
            ...
            self.images.append(torch.as_tensor(norm_data).float())
...
class RandomRGBSampleViewsDataset:
    ...
    def collate(self, index):
        ...
        norm_data = im_data / 255.0
        norm_data = torch.as_tensor(norm_data).float()
