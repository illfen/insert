...
class StableDiffusion(nn.Module):
    def __init__(self, opt, device, sd_path, sd_version='2.0'):
        ...
        self.min_step = int(self.num_train_timesteps * self.opt.sd_min_step_end)
    ...
    def train_dof_step_ssds(self, text_embeddings, pred_rgb, pose, ratio=0, guidance_scale=100, lamb = 1.0):
        ...
        if self.opt.sd_img_size == 512:
            ...
            pred_rgb = F.interpolate(pred_rgb, (1024, 1024), mode='bilinear', align_corners=False)

        if ratio > 0:

            max_s = int((self.opt.sd_max_step_end * ratio + self.opt.sd_max_step_start *(1-ratio))  * self.num_train_timesteps)
            # t = torch.randint(500, 750, [batch_size], dtype=torch.long, device=self.device)
        ...
        with torch.no_grad():
            ...
            text_embeddings = text_embeddings.repeat(2, 1, 1)
